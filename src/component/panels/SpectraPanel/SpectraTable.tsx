...
export function SpectraTable(props: SpectraTableProps) {
  ...
  const selectContextMenuHandler = useCallback(
    async (option: any, spectrum: Spectrum) => {
      const { id } = option;
      switch (id) {
  ...
  function handleActiveRow(row: TanStackTableRow<Spectrum>) {
  ...
  const tableColumns = useMemo(() => {
    ...
    // TODO: use column visibility feature
    ...
    for (const col of visibleColumns) {
      ...
      if (name && COLUMNS[name]) {
        ...
        const cell: TanStackTableColumn<Spectrum> = {
          header: () => <ColumnHeader label={col.label} col={col} />,
          accessorFn: (value) => getValueByPath(value, jpath, format),
          cell: ({ getValue }) => {
  ...
  function handleSortEnd(data: any, isTabledSorted?: boolean) {
    if (isTabledSorted) {
  ...
  function handleRowStyle(row: TanStackTableRow<Spectrum>) {
    return {
      base: activeSpectraSet.has(row.original.id) ? { opacity: 0.2 } : {},
      activated: { opacity: 1 },
    };
  }
