...
export function FidCanvas() {
  ...
  const data = useSpectrum(emptyData) as Datum2D;

  const canvasRef = useRef<HTMLCanvasElement>(null);
  console.log(data);
  useEffect(() => {
    const { minX, maxX, minY, maxY } = data.data;

    const matrix = matrixZPivotRescale(data.data.z, {
      max: 255,
